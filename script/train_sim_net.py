...
sys.path.append("../")
from datasets.hoia_prediction import HOIA
...
def train(continue_to_train):
    ...
    for epoch in range(start_epoch-1, num_epoch):
        ...
        if epoch % 10 == 0: # save every 10 epoch
            # Path("./checkpoint/").mkdir(parents=True, exist_ok=True)
            path_ckpt = os.path.join(path_ckpt, 'sim_mask_net_{}.pth'.format(epoch))
