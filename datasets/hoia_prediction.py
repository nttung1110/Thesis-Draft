...
def refine_box(h_box, o_box, img_path):
    ...
    return norm_out_h_box, norm_out_o_box, h_region, hom_region
...
class HOIA(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if exist:
            ...
            feat = np.concatenate((h_feat, hom_feat))
            # save to npy array
            np.save(path_feature, feat)

        input = np.concatenate([norm_out_h_box, norm_out_o_box, list(h_feat[0]), list(o_feat[0]), list(hom_feat[0])], 0)
...
class HOIA_infer(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        norm_h_box, norm_o_box, h_region, hom_region = refine_box(h_box, o_box, self.img_path)
        obj_cat_id = int(o_annot["category_id"])
        o_feat = encode_onehot(obj_cat_id)


        h_feat = get_vector(h_region)
        hom_feat = get_vector(hom_region)

        input = np.concatenate([norm_h_box, norm_o_box, list(h_feat[0]), list(o_feat[0]), list(hom_feat[0])])
        input = input.astype(np.float32)
