...
def pair_matching_model(h_annots, o_annots, model_matching, device, img_path, kp_dict):
    ...
    for idx_h, each_h in enumerate(h_annots):
        ...
        if idx_h == proposed_h_idx_match:
            ...
            pdb.set_trace()
            atten_weights = atten_weights_dict[str(proposed_h_idx_match)+"_"+str(proposed_o_idx_match)]
            
            chosen_o = o_annots[proposed_o_idx_match]
            true_idx_h = each_h["true_idx"]
            true_idx_o = chosen_o["true_idx"]
            pair_match.append((true_idx_h, true_idx_o, score_matching, atten_weights))
...
def infer_debug_images(path_json_detect, path_image_folder, path_json_out_res, path_json_pose, co_mat):
    # load model predict
    print("Loading model")
    ...
    file_name = "test_000015.png"
    instance = list(filter(lambda k:k["file_name"] == file_name, data))[0] 
    ...
    for each_pair in all_pair_res:
        # visualize matching results and attentional weight
...
if __name__ == '__main__':
    ...
    infer_debug_images(path_json_detect, path_image_folder, path_json_out, path_json_pose, co_mat)
